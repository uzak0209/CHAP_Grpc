					...
					return (
						<>
							...
							{eventsQuery.data?.events
								?.filter(
									(ev) => uiState.selectedCategory.toString() == ev.contentType,
								)
								.map((ev: V1Event) =>
							...
							{threadsQuery.data?.threads
								?.filter(
									(th) => uiState.selectedCategory.toString() == th.contentType,
								)
								.map((th: V1Thread) =>
							...
							{postsQuery.data?.posts
								?.filter(
									(po) => uiState.selectedCategory.toString() == po.contentType,
								)
								.map((po: V1Post) =>
									po.lat !== undefined && po.lng !== undefined ? (
										<Marker
											key={po.id}
											position={[po.lat, po.lng]}
											icon={blueIcon}
										>
											<Popup>
												<div className="min-w-[160px]">
